...
import { updateSectionSchema } from '../../validators/section.validators';
...
export const SectionEditPage: React.FC = () => {
  ...
  // Load configuration options & initial section data
  useEffect(() => {
    const initializePage = async () => {
      setFetching(true);
      try {
        const [semRes, teachRes, sectionRes] = await Promise.all([
        ...
        if (sectionRes.data?.status === 'success') {
          const sec = sectionRes.data.data;
          // Pre-populate fields
          reset({
            code: sec.code,
            name: sec.name,
            semesterId: sec.semesterId,
            classAdvisorId: sec.classAdvisorId,
            capacity: sec.capacity,
            currentStrength: sec.currentStrength,
            shift: sec.shift,
            status: sec.status,
        ...
        toast.error('Could not locate the class section or meta configuration.');
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      if (response.data?.status === 'success') {
        toast.success('Section modifications saved successfully!');
        navigate(ROUTES.SECTIONS);
      }
    } catch (err: any) {
      console.error('Section modification error:', err);
      toast.error(err.response?.data?.message || 'Failed to update section. Verify details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageContainer>
      <div className="mb-6 flex items-center gap-3" id="section-edit-header">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            Modify configuration fields, alter capacity benchmarks, or transfer student advisor assignments.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card className="p-6 border border-slate-200 bg-white shadow-xs rounded-2xl">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" id="edit-section-form">
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                ...
                >
                  ...
                  {loading ? 'Saving...' : 'Save Section Changes'}
                </Button>
              </div>
            </form>
          </Card>
