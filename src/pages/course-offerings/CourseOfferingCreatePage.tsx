...
import { createCourseOfferingSchema } from '../../validators/course-offering.validators';
...
export const CourseOfferingCreatePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLookups = async () => {
      try {
        ...
        console.error('[CourseOfferingCreate] Fetch lookup error:', err);
        toast.error('Failed to load pairing dependencies');
      }
    };
    fetchLookups();
  }, []);
  ...
  }, [watchedDeptId, lookups.programs, setValue]);
  ...
  useEffect(() => {
    ...
    setValue('semesterId', 0);
    setValue('sectionId', 0);
  }, [watchedProgId, lookups.semesters, setValue]);
  ...
  useEffect(() => {
    ...
    setValue('sectionId', 0);
  }, [watchedSemId, lookups.sections, setValue]);

  const onSubmit = async (data: any) => {
    setLoading(true);

    // Manual date validation before API call
    ...
    try {
      ...
      console.error('[CourseOfferingCreate] Error:', err);
      const errMsg = err.response?.data?.message || 'Failed to offer course';
      toast.error(errMsg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <PageContainer
      title="Offer New Subject"
      description="Pair a subject with a section, assign a teacher, and schedule the sessions."
      action={
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-4xl flex flex-col gap-6" id="course-offering-create-form">
        ...
        <Card id="card-pairing-info" className="p-6 flex flex-col gap-5">
          <div className="border-b border-gray-100 pb-3">
            ...
            <p className="text-xs text-gray-400 mt-1">Select the target program, section, and subject pairing.</p>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span>Confirm Course Offering</span>
              </>
            )}
          </Button>
        </div>
...
export default CourseOfferingCreatePage;
