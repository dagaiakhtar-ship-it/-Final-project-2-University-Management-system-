...
import { ArrowLeft, Save, AlertCircle, Calendar, HelpCircle, CheckCircle } from 'lucide-react';
...
export const TimetableEditPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialOptions = async () => {
      try {
        ...
        // Once lookups are ready, load the current record
        if (id) {
          fetchAndPopulateRecord(id);
        }
      } catch (err) {
  ...
  const fetchAndPopulateRecord = async (uuid: string) => {
    setLoading(true);
    try {
      ...
      if (entry) {
        ...
        // We fetch section details to get semester/program/dept IDs
        const secRes = await apiClient.get(`/sections/${sectionId}`);
        const secData = secRes.data.data;
        
        const semesterId = secData?.semesterId;
        const programId = secData?.semester?.programId;
        const departmentId = secData?.semester?.program?.departmentId;

        setSelectedDept(departmentId?.toString() || '');
        setSelectedProg(programId?.toString() || '');
        setSelectedSem(semesterId?.toString() || '');
        setSelectedSection(sectionId?.toString() || '');
        setSelectedCourseOffering(entry.courseOfferingId?.toString() || '');

        // Fetch Cascaded Lists for current selections so options populate correctly
        if (departmentId) {
          const progRes = await apiClient.get('/programs', { params: { departmentId } });
          setPrograms(progRes.data.data?.programs || progRes.data.data || []);
        }
        if (programId) {
          const semRes = await apiClient.get('/semesters', { params: { programId } });
          setSemesters(semRes.data.data?.semesters || semRes.data.data || []);
        }
        if (semesterId) {
          const sectionRes = await apiClient.get('/sections', { params: { semesterId } });
          setSections(sectionRes.data.data?.sections || sectionRes.data.data || []);
        }
        if (sectionId) {
          const coRes = await apiClient.get('/course-offerings', { params: { sectionId } });
          setCourseOfferings(coRes.data.data?.courseOfferings || coRes.data.data || []);
        }

        // Fetch Building & Rooms
        const buildingId = entry.room?.buildingId;
        setSelectedBuilding(buildingId?.toString() || '');
        if (buildingId) {
          const roomRes = await apiClient.get('/rooms', { params: { buildingId } });
          setRooms(roomRes.data.data?.rooms || roomRes.data.data || []);
        }

        // Format dates correctly for HTML5 Inputs (YYYY-MM-DD)
        ...
        reset({
          ...
          weeklyRepeat: entry.weeklyRepeat,
          effectiveFrom: formatForInput(entry.effectiveFrom),
          effectiveTo: formatForInput(entry.effectiveTo),
          status: entry.status,
          notes: entry.notes || '',
        });
      }
    } catch (err) {
      console.error('Failed to fetch existing class session', err);
  ...
  useEffect(() => {
    if (!selectedDept || loading) return;
  ...
  useEffect(() => {
    if (!selectedProg || loading) return;
  ...
  const onSubmit = async (data: FormValues) => {
    setSubmitting(true);
    try {
      await apiClient.put(`/timetable/${id}`, data);
      toast.success('Lecture session scheduled updated successfully!');
  ...
  if (loading) {
    return (
      <PageContainer title="Modify Class Schedule">
        <div className="flex items-center justify-center py-20" id="edit-loader">
  ...
  return (
    <PageContainer
      title="Modify Class Schedule"
      description="Update location, period, or status directives for this scheduled class session."
    >
      <div className="flex flex-col gap-6" id="timetable-edit-viewport">
        {/* Back Link */}
        <div className="flex">
