...
import { updateProgramSchema } from '../../validators/program.validators';
import { ArrowLeft, Check, RefreshCw, AlertCircle, HelpCircle } from 'lucide-react';
...
export const ProgramEditPage: React.FC = () => {
  ...
  const { id } = useParams<{ id: string }>(); // This represents the uuid parameter
  ...
  const {
    ...
    resolver: zodResolver(updateProgramSchema) as any,
  });

  // 1. Fetch initial configuration options (coordinators and departments)
  useEffect(() => {
  ...
  // 2. Fetch the current program record to reset form fields
  useEffect(() => {
    const fetchProgram = async () => {
      ...
      try {
        ...
        console.error('Failed to load program details for edit form:', err);
        setSubmitError('Failed to fetch the academic program data.');
      } finally {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const cleanData = {
        ...
        totalSemesters: data.totalSemesters ? Number(data.totalSemesters) : undefined,
      ...
      const response = await apiClient.put(`/programs/${id}`, cleanData);
      if (response.data?.status === 'success') {
      ...
      setSubmitError(
        ...
        'Failed to save updates to the program. Please try again.'
      );
    }
  };

  const isPageLoading = loadingInitial || fetchingRecord;
    ...
    return (
      <PageContainer>
        <div className="flex items-center justify-center min-h-[400px]">
          ...
          <span className="ml-3 text-gray-500 font-mono text-sm">Fetching program configurations...</span>
        </div>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      {/* Header Block */}
      <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-center md:justify-between" id="program-edit-header-block">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Update program regulations, description, department affiliation, or appointed coordinator.
          </p>
        </div>
        <div>
      ...
      {submitError && (
        <div className="mb-6 bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 flex items-start gap-3 text-sm animate-fade-in" id="program-edit-submit-error">
          <AlertCircle className="w-5 h-5 text-red-600 shrink-0 mt-0.5" />
          <div>
            <span className="font-semibold">Error Occurred:</span> {submitError}
          </div>
        </div>
      )}

      {/* Main Editing Card Form */}
      <Card className="p-6 md:p-8 max-w-4xl shadow-sm border border-gray-100" id="program-edit-form-card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input
                ...
                className="uppercase"
                disabled // Keep code disabled on edit to align with standard update schema rules
              />
            </div>
